...
(wp,ws,gstop,gpass)=(0.055,1e-10,100,0.003)
b,a = signal.iirdesign(wp = wp, ws=ws, gstop= gstop, gpass=gpass, ftype='cheby2')
filtyya=signal.lfilter(b,a,yya)

l=sum([1 for x in xxa if x<9.5])   
b,a = signal.iirdesign(wp = 0.08, ws= 1e-10, gstop= 10, gpass=0.001, ftype='cheby1')
filtyya[strt:l]=signal.lfilter(b,a,filtyya[strt:l])
"""filtyya=list()
...
if enableplot:
    ...
    pl.title("wp=%g, ws=%g, gstop=%g, gpass=%g"%(wp,ws,gstop,gpass))
    pl.show()
