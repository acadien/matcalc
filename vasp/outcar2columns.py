...
for ocf,output in zip(outcarFiles,outputFiles):
    ...
    data=["Column file generated by %s.\n"%(sys.argv[0].split("/")[-1]),
          ...
          "NION=%d\n"%(natom),
          "\n",
          "Step Temp cohEng kinEng totEng enthalpy Volume Pressure Pxx Pyy Pzz Pxy Pxz Pyz\n"]
    
    data+=["\t".join(map(str,[s,t,c,k,tot,h,v,p,xx,yy,zz,xy,xz,yz]))+"\n" \
               for s,t,c,k,tot,h,v,p,xx,yy,zz,xy,xz,yz in \
               zip(steps,temperatures,cohEnergies,kinEnergies,totEnergies,\
                       enthalpies,volumes,extPressures,xx,yy,zz,xy,xz,yz)]
