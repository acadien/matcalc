...
def unfrac(vec,mat):
    return linalg.solve(mat.T,array(vec))[:]

#Per atom optional should be a list of strings, 1 for each atom in each string
#Assumes types for only 1 config (unchanging types)
def writeDump(dumpfile,basis,types,atomConfigs,atomOptionals,atomOptionalsHead):
    TShead="ITEM: TIMESTEP\n"

    nAtom=len(types)
    NAhead="ITEM: NUMBER OF ATOMS\n%d\n"%nAtom

    #BBhead="ITEM: BOX BOUNDS xy xz yz xx yy zz\n"
    ...
    AThead="ITEM: ATOMS id type xs ys zs %s\n"%atomOptionalsHead

    ids= map(str,range(nAtom))
    types = map(str,types)

    dump = open(dumpfile,"w")
    A=matrix(basis)

    temp =[atomConfigs]+atomOptionals
    i=0
    for ensemb in itertools.izip(*temp):
        head = TShead + "%d\n"%i + NAhead + BBhead + AThead
...
def basis(dump):
    grepResults = subprocess.check_output("head -n 10000 %s | grep -b ITEM:\ BOX\ BOUNDS "%dump,shell=True).split("\n")
    b = [int(i.split(":")[0]) for i in grepResults if len(i)>2]
