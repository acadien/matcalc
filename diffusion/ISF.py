...
def ISFSelf(atoms,basis,steps=None,nqVecs=1,qmax=3.32,nStep=250):#qmax (first peak=1.9, second peak=3.32)
    atoms = array(atoms)
    basis = array(basis)
    nTime = atoms.shape[0]
    nAtom = atoms.shape[1]
...
def ISFFull(atoms,basis,steps=None,nqVecs=3,qmax=3.32,nStep=250):#kmax (first peak=1.9, second peak=3.32)
    atoms = array(atoms)
    basis = array(basis)
    nTime = atoms.shape[0]
    nAtom = atoms.shape[1]
...
"
    ...
    if isfType == "total":
        steps,isfs = ISFFull(atomsTime,basis,nqVecs=3,nStep=nStep)
        ylab = "$F_T(q,t)$"
    elif isfType == "self":
        steps,isfs = ISFSelf(atomsTime,basis,nqVecs=3,nStep=nStep)
        ylab = "$F_S(q,t)$"
    ...
    if isfType == "total":
        pl.title("Intermediate Scattering Function")
    pl.plot(steps,isfs)
