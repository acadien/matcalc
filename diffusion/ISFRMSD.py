...
def ISFSelf(atoms,basis,rmsd,steps=None,nqVecs=1,qmax=3.32,nStep=250,criteria=None):#qmax (first peak=1.9, second peak=3.32)
    atoms = array(atoms)
    basis = array(basis)
    nTime = atoms.shape[0]
    nAtom = atoms.shape[1]
...
def ISFFull(atoms,basis,rmsds,steps=None,nqVecs=3,qmax=3.32,nStep=250,criteria=None):#kmax (first peak=1.9, second peak=3.32)
    atoms = array(atoms)
    basis = array(basis)
    nTime = atoms.shape[0]
    nAtom = atoms.shape[1]
...
"
    ...
    if isfType == "total":
        steps,isfs = ISFFull(atomsTime,basis,rmsdTime,nqVecs=3,nStep=nStep,criteria=criteria)
        ylab = "$F_T(q,t)$"
        pl.title("Intermediate Scattering Function RMSD"+criteria)
    elif isfType == "self":
        steps,isfs = ISFSelf(atomsTime,basis,rmsdTime,nqVecs=3,nStep=nStep,criteria=criteria)
        ylab = "$F_S(q,t)$"
