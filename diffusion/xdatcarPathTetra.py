...
xdatcarFileName,tetraFile,atomRequested = sys.argv[1],sys.argv[2],int(sys.argv[3])
...
##Parsing the tetra file for tetrahedrality
atomTetra = []
for i,line in enumerate(open(tetraFilie,"r")):
    line = line.split()
    if len(line)<20:
        continue
    atomTetra.append(float(line[atomRequested+1]))
print atomTetra
exit(0)
...
def coarseGrain(x, y, z,cg=10):
    cgx = [sum(x[i:i+cg])/cg for i in range(int(len(x)/cg))]
    cgy = [sum(y[i:i+cg])/cg for i in range(int(len(x)/cg))]
    cgz = [sum(z[i:i+cg])/cg for i in range(int(len(x)/cg))]
    return cgx,cgy,cgz

atomTraj = coarseGrain(atomTraj[0],atomTraj[1],atomTraj[2])

def colorLine(x, y, z, c=None, cmap=plt.get_cmap('brg'),alpha=0.4):
    if c==None:
...
ax.add_collection(colorLine(atomTraj[0],atomTraj[1],atomTraj[2]))#,atomTetra)
