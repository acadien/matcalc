...
fdb=-1
if len(sys.argv)>=3:
    headers=[line for line in open(sys.argv[2],"r").readlines() if "ifconf" in line]
    fdb=["/".join(line.split("home")[-1].split("OUTCAR")[0].split("/")[6:]) for line in headers]
...
if fdb!=-1:
    ...
    slowDelE=[i for i,j in zip(absdelE,fdb) if "slow" in j]
