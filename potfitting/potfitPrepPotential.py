...
#    l[0]=0.0 #sometimes F(0)!=0 this is an error from potfit
    xF=genXs(l)
...
#        x=append(x,x[-1]+step)
#        y=append(y,y[-1]+primes[1]*step)

        return x,y

    def appendPrimes2(x,y,primes,step):
#        x=insert(x,[1],[x[0]+step])
#        y=insert(y,[1],[y[0]+primes[0]*step])
        x=insert(x,0,[x[0]-step])
        y=insert(y,0,[y[0]-primes[0]*step])
...
if len(sys.argv)==4:
    ...
    Phi=map(float,potdata[6+nrho:6+nr+nrho])
    Rho=map(float,potdata[6+nr+nrho:6+2*nr+nrho])
