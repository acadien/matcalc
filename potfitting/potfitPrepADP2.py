...
def usage():
    print "%s <pf_end_??.adp> <info file> <output potential> <optional:lammps potential>"%sys.argv[0].split("/")[-1]
...
potentialInfo = open(sys.argv[2]).readlines()
...
for i in range(nElem):
    ...
    potential.append(potentialInfo[i])#"atomNumber mass latConst latType"
...
for LW in LWs:
    potential+="".join(map(lambda x:"%6.6e\n"%x,LW))
open(sys.argv[3],"w").writelines(potential)
...
if len(sys.argv)==5:
    ...
    potdata=open(sys.argv[4],"r").readlines()
    #Write element specific data
#    potdata[3]="1 Ge\n"
